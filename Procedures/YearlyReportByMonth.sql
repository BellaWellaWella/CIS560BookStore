...
CREATE OR ALTER PROCEDURE YearlyReportByMonth
	@ReportYear INT
AS
SELECT SUM(BFS.Price)AS TotalSales, 
	   COUNT(BFS.BookForSaleID)AS BooksSold, 
	   ROUND(SUM(BFS.Price)/COUNT(BFS.BookForSaleID),2)AS AveragePrice,
	   Month(S.DatePurchased)AS Monthly
FROM Sales S
